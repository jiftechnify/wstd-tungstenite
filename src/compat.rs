    ...
    S: AsyncRead + Unpin,
{
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        match self.with_context(ContextWaker::Read, |ctx, mut stream| {
            let mut fut = stream.as_mut().get_mut().read(buf);
            // Safety: fut is created from a pinned stream and won't be moved
            unsafe { Pin::new_unchecked(&mut fut) }.poll(ctx)
        }) {
    ...
    S: AsyncWrite + Unpin,
{
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        match self.with_context(ContextWaker::Write, |ctx, mut stream| {
            let mut fut = stream.as_mut().get_mut().write(buf);
            // Safety: fut is created from a pinned stream and won't be moved
            unsafe { Pin::new_unchecked(&mut fut) }.poll(ctx)
        }) {
            Poll::Ready(r) => r,
    ...
    fn flush(&mut self) -> std::io::Result<()> {
        match self.with_context(ContextWaker::Write, |ctx, mut stream| {
            let mut fut = stream.as_mut().get_mut().flush();
            // Safety: fut is created from a pinned stream and won't be moved
            unsafe { Pin::new_unchecked(&mut fut) }.poll(ctx)
        }) {
            Poll::Ready(r) => r,
